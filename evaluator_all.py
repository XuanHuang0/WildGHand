...
class Evaluator:
    ...
    @staticmethod
    def _compute_psnr_cut(img_pred, img_gt, mask_at_box):
    ...
    def _compute_ssim(self, rgb_pred, rgb_gt, input_imgs, mask_at_box, human_idx, frame_index, view_index):
        ...
        human_dir = os.path.join(self.result_dir, human_idx,'all')
        ...
        cv2.imwrite(os.path.join(gt_dir, f'frame{frame_index}_view{view_index}_gt.png'), (rgb_gt[..., [2, 1, 0]]*255))
        cv2.imwrite(os.path.join(pred_dir, f'frame{frame_index}_view{view_index}.png'), (rgb_pred[..., [2, 1, 0]]*255))
        cv2.imwrite(os.path.join(pred_dir, f'frame{frame_index}_view{view_index}_{ssim}.png'), (rgb_pred[..., [2, 1, 0]]*255))
        ...
        for view in range(input_imgs.shape[0]):
            cv2.imwrite(os.path.join(input_dir, f'frame{frame_index}_t_0_view_{view_index}.png'), input_imgs[view])
    ...
    def compute_score(self, rgb_pred, rgb_gt, input_imgs, mask_at_box, human_idx, frame_index, view_index, ka_xy=None,vert_vis=None, vert_xy=None, joint_xy=None, fake_vis_pred=None, real_vis_pred=None, vis_img_gt=None, msk=None):
        ...
        # psnr_cut = self._compute_psnr_cut(rgb_pred, rgb_gt, mask_at_box)
