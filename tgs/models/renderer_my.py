...
class Camera:
    def __init__(self, w2c, intrinsic, FoVx, FoVy, height, width, znear, zfar, world_view_transform, trans=np.array([0.0, 0.0, 0.0]), scale=1.0) -> None:
        self.FoVx = FoVx
        self.FoVy = FoVy
        self.height = height
        self.width = width
        ...
        # print(self.world_view_transform.shape)
    ...
    @staticmethod
    def from_w2c(w2c, intrinsic, height, width, znear, zfar, world_view_transform):
        FoVx, FoVy = intrinsic_to_fov(intrinsic, w=torch.tensor(width, device=w2c.device), h=torch.tensor(height, device=w2c.device))
        return Camera(w2c=w2c, intrinsic=intrinsic, FoVx=FoVx, FoVy=FoVy, height=height, width=width, znear=znear, zfar=zfar, world_view_transform=world_view_transform)
    ...
    def forward_single_batch(
        ...
        for w2c, intrinsic in zip(w2cs, intrinsics):
            out_list.append(self.forward_single_view(
                                gs, 
                                Camera.from_w2c(w2c = w2c, intrinsic = intrinsic, height = height, width = width, znear = znear, zfar = zfar, world_view_transform = world_view_transform),
                                #w2c, intrinsic, height, width, znear, zfar, world_view_transform
                                background_color
                            ))
        ...
        **kwargs):
        ...
        for b in range(batch_size):
            out_list.append(self.forward_single_batch(
                ...
                znear=znear,
                zfar=zfar,
                world_view_transform=world_view_transform[b],
