...
def weights_init(layer):
    ...
    if classname.find('Conv2d') != -1:
        ...
        nn.init.xavier_uniform_(layer.weight.data)
        # nn.init.constant_(layer.weight.data, 0.0)
        if layer.bias is not None:
            nn.init.constant_(layer.bias.data, 0.0)

def weights_init_0(layer):
    classname = layer.__class__.__name__
    # print(classname)
    if classname.find('Conv2d') != -1:
        nn.init.xavier_uniform_(layer.weight.data)
...
class Linear_fc(nn.Module):
    def __init__(self, f_dim_in, f_dim_out):
        super().__init__()
        self.ff1 = nn.Linear(f_dim_in, f_dim_in)
        self.ff2 = nn.Linear(f_dim_in, f_dim_out)
...
class additional_features_fc_0(nn.Module):
    def __init__(self, f_dim_in, f_dim_out):
        super().__init__()
        self.ff1 = MLP_block(f_dim_in, f_dim_out)
        # self.ff1 = nn.Linear(f_dim_in, f_dim_out)
        # self.ff2 = nn.Linear(f_dim_out, f_dim_out)

        for m in self.modules():
            weights_init_0(m)

        # for m in self.ff1.modules():
        #     weights_init_0(m)
        
        # for m in self.ff2.modules():
        #     weights_init(m)
...
# class id_features_fc(nn.Module):
#     def __init__(self, f_dim_in, f_dim_out):
#         super().__init__()
#         self.ff1 = MLP_block(f_dim_in, f_dim_out)
