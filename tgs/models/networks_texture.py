...
class TriplaneUpsampleNetwork(BaseModule):
    ...
    class Config(BaseModule.Config):
        ...
        out_channels_2: int = 256
        out_channels_3: int = 128
        out_channels_4: int = 64
        n_plane: int = 2
    cfg: Config
    ...
    def configure(self) -> None:
        ...
        self.upsample_2 = nn.ConvTranspose2d(
            self.cfg.out_channels, self.cfg.out_channels_2, kernel_size=2, stride=2
        )
        self.upsample_3 = nn.ConvTranspose2d(
            self.cfg.out_channels_2, self.cfg.out_channels_3, kernel_size=2, stride=2
        )
        self.upsample_4 = nn.ConvTranspose2d(
            self.cfg.out_channels_3, self.cfg.out_channels_4, kernel_size=2, stride=2
        )

    def forward(
        self, triplanes: Float[Tensor, "B 1 Ci Hp Wp"]
    ) -> Float[Tensor, "B 1 Co Hp2 Wp2"]:
        triplanes_32 = rearrange(triplanes, "B Np Ci Hp Wp -> (B Np) Ci Hp Wp", Np=self.cfg.n_plane)
        triplanes_64 = self.upsample(triplanes_32)
        triplanes_128 = self.upsample_2(triplanes_64)
        triplanes_256 = self.upsample_3(triplanes_128)
        triplanes_512 = self.upsample_4(triplanes_256)

        triplanes_512 = rearrange(triplanes_512, "(B Np) Co Hp Wp -> B Np Co Hp Wp", Np=self.cfg.n_plane)
        triplanes_512 = torch.cat([triplanes_512[:,0], triplanes_512[:,1]], dim=-1).unsqueeze(1)

        triplanes_256 = rearrange(triplanes_256, "(B Np) Co Hp Wp -> B Np Co Hp Wp", Np=self.cfg.n_plane)
        triplanes_256 = torch.cat([triplanes_256[:,0], triplanes_256[:,1]], dim=-1).unsqueeze(1)

        triplanes_128 = rearrange(triplanes_128, "(B Np) Co Hp Wp -> B Np Co Hp Wp", Np=self.cfg.n_plane)
        triplanes_128 = torch.cat([triplanes_128[:,0], triplanes_128[:,1]], dim=-1).unsqueeze(1)

        triplanes_64 = rearrange(triplanes_64, "(B Np) Co Hp Wp -> B Np Co Hp Wp", Np=self.cfg.n_plane)
        triplanes_64 = torch.cat([triplanes_64[:,0], triplanes_64[:,1]], dim=-1).unsqueeze(1)

        triplanes_up = [triplanes_512, triplanes_256, triplanes_128, triplanes_64]
