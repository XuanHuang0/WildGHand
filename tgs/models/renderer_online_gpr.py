...
class GS3DRenderer(BaseModule):
    ...
    def forward_gs(self, x, p, x_s):
        if self.cfg.mlp_network_config is not None:
            x = self.mlp_net(x)
            # if x_s is not None:
            #     x_s = self.mlp_net_s(x_s)
        return self.gs_net(x, p, x_s)
    ...
    def forward_single_batch(
        ...
        gs, v_s = self.forward_gs(gs_hidden_features_copied, query_points_valid, shade_features)
        out_list = []
