...
class GS3DRenderer(BaseModule):
    ...
    def forward_single_view(self,
        ...
        # print(int(viewpoint_camera.height))
        # print(int(viewpoint_camera.width))
    ...
    def forward_single_batch(
        ...
        # # if if_gs_valid.min() < 0.5:
        # print("if_gs_valid")
        # print(query_points_valid.shape)
        gs: GaussianModel = self.forward_gs(gs_hidden_features, query_points)
        out_list = []
       
        vert_uv, vert_d, intermediates_vert = get_uvd(query_points, vert3d_uv[0], face_uv, face_uv_xy)
        # normalize it to [-1, 1]
        ...
        # # + color_w.view(-1,16,3)[0,1,:] -1
        # # color_w0[...,1024:]=color_w0[...,1024:]*color_w.view(-1,16,3)[0,2,:] + color_w.view(-1,16,3)[0,3,:] -1
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        # gs_hidden_features_texture_rbg = self.query_triplane_texture(vert_uv, texture_rgb)
        # print("rgb")
        # print(gs_hidden_features_texture_rbg.shape)

        # mesh_v = trimesh.Trimesh(vertices=query_points[0].detach().cpu().numpy(), faces=face[0].detach().cpu().numpy(), process=False)
        # mesh_v.visual.vertex_colors = gs_hidden_features_texture_rbg[0].detach().cpu().numpy()
        # # trimesh.visual.color.ColorVisuals(mesh=mesh_v, vertex_colors=local_features_proj[0].detach().cpu().numpy()*255)
        # mesh_v.export('mesh_texture.obj')
        ...
        # print("gs_hidden_features.shape")
        # print(gs_hidden_features.shape)
        # n_points = gs_hidden_features.shape[1]
        # print(int(n_points/2))
        ...
        if mink_idxs_inter.max()>0:
            ...
            for b in range(batch_size):
                ...
                # print(gs_hidden_features_b.shape)
                # print(mink_idxs_inter_b.shape)
                # print(gs_hidden_features_b[mink_idxs_inter_b].shape)
                if mink_idxs_inter_b.max()>0:
                    if mink_idxs_inter.shape[1] >30000:
                        ...
                        for p in range(n_part):
                            ...
                            print("mink_idxs_inter_part")
                            print((gs_hidden_features_b[mink_idxs_inter_b_part]).shape)
                    else:
                        gs_hidden_features_b[mink_idxs_inter_b] = self.self_attn_layer(gs_hidden_features_b[mink_idxs_inter_b].unsqueeze(0)).squeeze(0)
                        print("mink_idxs_inter")
                        print(gs_hidden_features_b[mink_idxs_inter_b].shape)
                gs_hidden_features_list.append(gs_hidden_features_b)
            gs_hidden_features = torch.cat(gs_hidden_features_list, dim=0)
        # Rf = self.R_self_attn_layer(gs_hidden_features[:,:int(n_points/2)])
        # Lf = gs_hidden_features[:,int(n_points/2):]
        # # query_points_close = 
        # Rf = gs_hidden_features[:,:int(n_points/2)]
        # Lf, Rf = self.inter_attn(Lf, Rf)
        ...
        # print("aux_cam_w2c")
        # print(aux_cam_w2c[0,0].shape)
        # print(aux_cam_intrinsic[0,0].shape)
        # print()
