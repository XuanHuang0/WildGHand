...
class GSLayer(BaseModule):
    ...
    def forward(self, x, x_shade, pts, x_s):
        ret = {}
        for k, layer, layer_shade in zip(self.cfg.feature_channels.keys(), self.out_layers, self.out_layers_shade):
            v = layer(x)
        ...
        # print('v_pts.shape')
        # print(v_pts.shape)
        return GaussianModel(**ret), v_s, v_pts, ret
...
class GS3DRenderer(BaseModule):
    ...
    def forward_single_batch(
        ...
        gs, v_s, v_pts, gs_att = self.forward_gs(gs_hidden_features_tex, gs_hidden_features_shade, query_points, shade_features)
        print(v_pts.shape)
        out_list = []
