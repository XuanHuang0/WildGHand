...
class GSLayer(BaseModule):
    ...
    def forward(self, x, x_time, pts, x_s):
        ret = {}
        v_time_sum = 0
        if self.cfg.time_reg:
            rand_value = random.random()
            time_weight_weight = 0 if rand_value <= 0.3 else 1
        for k, layer, layer_time in zip(self.cfg.feature_channels.keys(), self.out_layers, self.out_layers_time):
            v = layer(x)
...
class GS3DRenderer(BaseModule):
    ...
    def forward_gs(self, x_tex, x_time, p, x_s):
        if self.cfg.mlp_network_config is not None:
            x_tex = self.mlp_net(x_tex)
        return self.gs_net(x_tex, x_time, p, x_s)
    ...
    def forward_single_batch(
        ...
        gs, v_s, v_pts, gs_att, time_weight, v_time_sum = self.forward_gs(gs_hidden_features_tex, time_features, query_points, shade_features)
        out_list = []
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        batch_size = gs_hidden_features_tex.shape[0]
