...
class GSLayer(BaseModule):
    ...
    def configure(self, *args, **kwargs) -> None:
        ...
        for key, out_ch in self.cfg.feature_channels.items():
            if (key == "shs" or key == "shs_bias") and self.cfg.use_rgb:
                out_ch = 3
    ...
    def forward(self, x, pts, x_s, ret_bias = {}):
        ret = {}
        for k, layer in zip(self.cfg.feature_channels.keys(), self.out_layers):
            v = layer(x)
            if k+'_bias' in ret_bias:
                print(k+'_bias')
                print(v.shape)
                print(ret_bias[k+'_bias'].shape)
                v = v + ret_bias[k+'_bias']
            if k == "rotation":
        ...
        return GaussianModel(**ret), v_s, v

    def forward_bias(self, x_b):
        ret = {}
        for k, layer in zip(self.cfg.feature_channels.keys(), self.out_layers):
            if "bias" in k:
                v = layer(x_b)
...
class GS3DRenderer(BaseModule):
    ...
    def configure(self, *args, **kwargs) -> None:
        ...
        if self.cfg.mlp_network_config is not None:
            ...
            self.mlp_net_b = MLP(mlp_in_b, self.cfg.gs_out.in_channels, **self.cfg.mlp_network_config)
        else:
            self.cfg.gs_out.in_channels = mlp_in
        self.gs_net = GSLayer(self.cfg.gs_out)

    def forward_gs(self, x, p, x_s, x_b):
        if self.cfg.mlp_network_config is not None:
            x = self.mlp_net(x)
            if x_s is not None:
                x_s = self.mlp_net_s(x_s)
            if x_b is not None:
                x_b = self.mlp_net_b(x_b)
            ret_bias = self.gs_net.forward_bias(x_b)
        return self.gs_net(x, p, x_s, ret_bias)
    ...
    def forward_single_batch(
        ...
        np, fdim = gs_hidden_features.shape
        gs, v_s, v = self.forward_gs(gs_hidden_features, query_points, shade_features, torch.cat([gs_hidden_features, time_feature.repeat(np,1)],dim=-1))
        out_list = []
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        if gs_hidden_features is None:
            ...
            gs_hidden_features = torch.cat(gs_hidden_features_texture_list,dim=-1)
            # print(gs_hidden_features.shape)
        ...
        for b in range(batch_size):
            ...
            if time_feature is not None:
                time_feature_b = time_feature[b]
            else:
                time_feature_b = time_feature
            out_list_input.append(self.forward_single_batch(
                gs_hidden_features=gs_hidden_features[b],
                shade_features=shade_features_b,
                time_feature=time_feature_b,
                query_points=query_points[b],
                w2cs=w2c_input[b],
