...
def render_img(verts, faces,R,T, fx, fy, px, py, mask_path='mask.jpg',image_size=(256,256),device='cuda:2'):
    ...
    tex = torch.zeros_like(verts.squeeze(0)) 
    tex[:, 2] = 1.0 # white
    ...
    im_silhouette[im_silhouette > 0] = 1  # Binarize segmentation mask

    # cv2.imwrite(mask_path, im_silhouette.detach().cpu().numpy()[0, ..., 3]*255)


    return im_silhouette[0, ..., 3]



def render_handy(verts, faces, uv_coords, texture_uv, R,T, fx, fy, px, py, mask_path='mask.jpg',image_size=(256,256),device='cuda:2'):
    ...
    tex = TexturesUV(maps=texture_uv.to(device), verts_uvs=uv_coords.unsqueeze(0).to(device), faces_uvs=faces.unsqueeze(0).to(device))
    mesh = Meshes(verts=[verts], faces=[faces], textures=tex)
    ...
    blend_params = BlendParams(sigma=1e-9, gamma=1e-9, background_color=(0.0, 0.0, 0.0))
    ...
    lights = DirectionalLights(device=device, direction=[[0, 0, -1]], ambient_color=[[0.3, 0.3, 0.3]], diffuse_color=[[0.5, 0.5, 0.5]], specular_color=[[0.1, 0.1, 0.1]])
    ...
    # shader.ambient_color = [[0.9, 0.9, 0.9]]
...
def render_handy2(verts_list, faces_list, uv_coords_list, texture_uv_list, R, T, fx, fy, px, py, 
                  mask_path='mask.jpg', image_size=(256,256), device='cuda:2'):

    device = torch.device(device)
    
    # 处理多个 Mesh
    meshes = []
    for verts, faces, uv_coords, texture_uv in zip(verts_list, faces_list, uv_coords_list, texture_uv_list):
        # 确保 faces 形状为 (F, 3)
        if faces.dim() == 3:
            faces = faces.squeeze(0)

        # 处理 TexturesUV，确保所有输入都符合形状要求
        tex = TexturesUV(maps=texture_uv, 
                         verts_uvs=uv_coords, 
    ...
    # **合并多个 Mesh**
    mesh_batch = join_meshes_as_scene(meshes).to(device)
    ...
    im_silhouette = silhouette_renderer(mesh_batch)
    im_silhouette[im_silhouette > 0] = 1  # 二值化分割掩码
...
#         tex = TexturesUV(maps=texture_uv, verts_uvs=uv_coords, faces_uvs=faces.unsqueeze(0))
#         meshes.append(Meshes(verts=[verts], faces=[faces], textures=tex))

#     # 合并多个 Mesh
#     mesh_batch = Meshes(
#         verts=[m.verts_packed() for m in meshes], 
#         faces=[m.faces_packed() for m in meshes], 
#         textures=[m.textures for m in meshes]
#     )
...
#     # 渲染最终图像
#     image = renderer(mesh_batch.to(device)) 
#     im_silhouette = silhouette_renderer(mesh_batch.to(device))
#     im_silhouette[im_silhouette > 0] = 1  # 二值化分割掩码

#     return image, im_silhouette[0, ..., 3]


# def render_handy2(verts, faces, uv_coords, texture_uv, R,T, fx, fy, px, py, mask_path='mask.jpg',image_size=(256,256),device='cuda:2'):
...
def render_msk(verts, faces, R,T, fx, fy, px, py, mask_path='vis2.jpg',image_size=(256,256),device='cuda:1'):
    ...
    return im_vis_rbg.permute(0,3,1,2)/255
