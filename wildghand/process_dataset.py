...
from wildghand.vis_util import draw_2d_skeleton
...
def run(i, image, img_path, joints_uv, or_img_path, rendered_mask=None):
    ...
    mask_bbox = ((((bbox_mask_left[...,None])+(bbox_mask_right[...,None]))>0.9)).astype("uint8")*255
    ...
    save_path_b = os.path.join(output_folder, '{:>04d}mask_bbox.png'.format(i))
    cv2.imwrite(save_path_b, mask_bbox)
    # save_path_r = os.path.join(output_folder, '{:>04d}mask_render.png'.format(i))
    # cv2.imwrite(save_path_r, rendered_mask)
    ...
    return mask_b, mask_bbox, np.array(bbox_inter)
...
if __name__ == "__main__": 
    ...
    for acount, aid in tqdm(enumerate(data_wild_video)):
        ...
        mask_sam_b, mask_bbox, bbox_inter = run(acount, input_img, 'mask.jpg', pj2d_42,'mask.jpg')
        print('output')
        ...
        data_wild_video[acount]['mask_bbox'] = mask_bbox
