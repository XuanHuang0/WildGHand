...
from wildghand.model_utils_mask_lr import compute_error, VGGLoss, LaplacianReg
...
import copy
import wildghand.evaluator as evaluator
# from .DINAR.discriminators.style_gan_v2 import Discriminator
import yaml
import wildghand.config as config
from wildghand.spatial import SpatialEncoder
...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def __init__(self, cfg):
        ...
        self.t_pointclouds = t_smpl_output_r.vertices
        ...
        print("vertex_num_upsampled",vertex_num_upsampled)
    ...
    def _forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.cfg.smpl_cfg['ignore_deform']:
            offsets_right = template_offsets
            ...
            out["verts_template"] = self.mano_layer_right.v_template[None].clone().detach()
            # out["shaped_verts"] = torch.cat([shaped_verts_right,shaped_verts_right],dim=-2)
            out["shaped_verts"] = shaped_verts_right
        ...
        if self.cfg.add_time_bias:
            ...
            # print(total_frame.shape) #([2, 1]) 
            # time_interval = torch.ones_like(total_frame) / total_frame
            time_interval = 1 / total_frame
            # print(time_interval.shape) #([2, 1]) 
            frame_id = batch["index"]
            # print(frame_id.shape) #([2]) 
            frame_t =  (frame_id*time_interval.squeeze(-1)).view(batch_size,-1)
            # frame_t =  (frame_id*time_interval).view(batch_size,-1)
            # print(frame_t.shape) #torch.Size([1, 1]) tensor([1, 2], device='cuda:0')     
            # print(frame_t) #tensor([[0.]], device='cuda:0')
            frame_t_sp = self.sp_encoder_time(frame_t.unsqueeze(1))

            # print("add_time_bias")
            # print(frame_t_sp.shape) #[2, 1, 18]) torch.Size([1, 1, 9])
            # print(id_feature.shape) #([2, 256])
            # print(mano_pose.shape) #[2, 2, 48]) 
            # print( mano_trans.shape) #([2, 2, 3])
...
class HandLightningModule(pytorch_lightning.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if "comp_rgb_input" in out:
            ...
            print("mano_mask_max:", str(frame_idx), out['mano_msk'].sum())

            if out['mano_msk'].sum() < 2500 :
                return None 
        ...
        mano_msk = out['tar_alpha_input'].permute(0, 2, 3, 1)[0].clone().detach().cpu().numpy()*255
        cv2.imwrite("./vis/"+expname+"/rgb"+str(frame_idx)+"_mano_msk.jpg", mano_msk)
        
        # print("mano_mask_max:", str(frame_idx), out['tar_alpha_input'].max())
    ...
    def test_step(self, batch, batch_nb):
        ...
        out["tar_alpha_input"] = out['mano_msk'].float().unsqueeze(1).clone()

        # out["tar_alpha_input"] = out['comp_mask_input'].float().mean(-1).unsqueeze(-1).squeeze(1).permute(0, 3, 1, 2)
        ...
        rendered_image = out["tex_cal_fine"].clone()
        ...
        cv2.imwrite(os.path.join(render_dir, f'frame{frame_index}_view{view_index}_pred_{psnr}_lr1.png'), (rgb_pred[..., [2, 1, 0]]*255))
        # cv2.imwrite(os.path.join(render_dir, f'frame{frame_index}_view{view_index}_input.png'), input_imgs[0])

        out["tex_cal_fine"][bbox_mask.repeat(1,3,1,1) == 0] =0
        rendered_image = out["tex_cal_fine"]
        rgb_pred = rendered_image.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
        cv2.imwrite(os.path.join(render_dir, f'frame{frame_index}_view{view_index}_pred_{psnr}_lr1_bbox.png'), (rgb_pred[..., [2, 1, 0]]*255))
...
if __name__ == "__main__":
    ...
    from wildghand.data_single import Dataset, load_cfg
    ...
    # model_tgs = TGS(cfg=cfg.system).to(device)
